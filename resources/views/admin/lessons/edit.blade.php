                ...
                <div class="lg:col-span-8">
                    <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <form method="POST" action="{{ route('admin.chapters.lessons.update', [$chapter->id, $lesson->id]) }}" enctype="multipart/form-data" class="p-6">
                            ...
                            {{-- Inisialisasi Data Alpine dengan Data Database --}}
                            <div x-data="{ 
                                type: '{{ old('type', $lesson->type) }}', 
                                videoSource: '{{ old('video_source', $lesson->video_source ?? 'upload') }}',
                                questions: {{ \Illuminate\Support\Js::from($lesson->questions ?? []) }},
                                addQuestion() {
                                ...
                                <div class="bg-slate-50 border border-slate-100 rounded-xl p-5 mb-6">
                                    ...
                                    <div x-show="type === 'video'" x-transition class="mb-6">
                                        ...
                                        @if($lesson->type == 'video')
                                            <div class="mb-5 p-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                                                <p class="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2">
                                                ...
                                                @elseif($lesson->video_source == 'youtube')
                                                    <div class="flex items-center gap-2 text-sm text-indigo-600 bg-indigo-50 p-2 rounded">
                                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"/></svg>
                                                        <a href="{{ $lesson->file_path }}" target="_blank" class="underline truncate">{{ $lesson->file_path }}</a>
                                                    </div>
                                                @endif
                                            </div>
                                        @endif
                                        ...
                                        <div x-show="videoSource === 'youtube'">
                                            <x-text-input class="block w-full" type="text" name="video_url" 
                                                :value="old('video_url', ($lesson->video_source == 'youtube' ? $lesson->file_path : ''))" 
                                    ...
                                    <div x-show="type === 'pdf'" x-transition style="display: none;" class="mb-6">
                                        @if($lesson->type == 'pdf' && $lesson->file_path)
                                            <div class="mb-4 flex items-center gap-2 text-sm bg-white p-3 rounded border border-slate-200">
                                                ...
                                                <span class="text-slate-600">File saat ini: <a href="{{ Storage::url($lesson->file_path) }}" target="_blank" class="text-indigo-600 underline font-bold">Lihat PDF</a></span>
                                            </div>
                                        @endif
                                        <label class="block text-sm font-bold text-slate-700 mb-2">Ganti PDF</label>
                                    ...
                                    {{-- Deskripsi / Konten (Tampil untuk Video, Text, dan Assignment) --}}
                                    <div x-show="['text', 'assignment', 'video'].includes(type)" x-transition>
                                        
                                        {{-- Info Box Khusus Assignment --}}
                                        ...
                                        <textarea id="content" name="content" class="block w-full border-gray-300 rounded-md shadow-sm h-40">{{ old('content', $lesson->content) }}</textarea>
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="mb-6 flex items-center justify-between p-4 bg-white border border-slate-200 rounded-lg hover:border-indigo-200 transition-colors">
                                            ...
                                            <label class="relative inline-flex items-center cursor-pointer">
                                                <input type="checkbox" name="show_results" value="1" class="sr-only peer" {{ $lesson->show_results ? 'checked' : '' }}>
                                                ...
                                                <template x-for="(q, index) in questions" :key="index">
                                                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-5 relative transition hover:border-purple-200 hover:shadow-sm">
                                                        ...
                                                        <div class="mb-4">
                                                            ...
                                                            {{-- Gunakan question_text dari DB jika ada (karena dari server key-nya question_text) --}}
                                                            <input type="text" :name="'questions['+index+'][text]'" x-model="q.question_text || q.text" class="w-full text-sm border-slate-300 rounded-lg focus:ring-purple-500 focus:border-purple-500" placeholder="Tulis pertanyaan di sini..." required>
                                                        </div>
                                                            ...
                                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                                <template x-for="opt in ['A', 'B', 'C', 'D']">
                                                                    <div class="flex items-center gap-2">
                                                                        ...
                                                                        {{-- Handle opsi dari DB (bisa null jika soal baru) --}}
                                                                        <input type="text" :name="'questions['+index+'][options]['+opt+']'" x-model="q.options ? q.options[opt] : ''" 
